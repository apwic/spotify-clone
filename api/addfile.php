...
if (($_POST['type']) != ("song")) {
    ...
    $target_filename = basename($_FILES["img_file"]["name"]);
    $target_filepath = $target_dir . $target_filename;
    
    if(!move_uploaded_file($_FILES["img_file"]["tmp_name"], $target_filepath)) {
    ...
    if (!file_exists($target_filepath)) {
        ...
        $result = ["status" => "error", "description" => "song already exists", $_FILES["file_lagu"]];
